...
pub use taxonomy_cli::*;

// The superseded template/typeset CLI remains below for manual verification.
#[cfg(any())]
mod legacy {
    ...
    use gitserious_app::{
        ...
        ConfigurationEditor, ConfigurationOrigin, GlobalConfigurationStore, InitOutcome,
        InitStatus, ProjectStateStore, RepositoryLocator, RepositoryRoot,
        built_in_effective_catalog, create_commit_with_template, delete_taxonomy, delete_template,
        delete_typeset, edit_configuration, fork_conventional, initialize_project,
        load_effective_catalog, template_origin,
    };
    ...
    {
        let template =
            match parse_identifier(template_text, "template", |text| TemplateId::new(text)) {
                Ok(id) => id,
                Err(error) => return write_operational_error(stderr, error),
            };
        let taxonomy_text =
    ...
    fn resolve_show_target(
        ...
        match kind {
            ConfigurationKind::Taxonomy => {
                let id = gitserious_core::TaxonomyId::new(identity).map_err(|error| {
                    format!("invalid taxonomy identifier {identity:?}: {error}")
                })?;
            ...
            ConfigurationKind::Typeset => {
                ...
                let taxonomy =
                    gitserious_core::TaxonomyId::new(taxonomy_text).map_err(|error| {
                        format!("invalid taxonomy identifier {taxonomy_text:?}: {error}")
                    })?;
                let id = gitserious_core::TypesetId::new(typeset_text).map_err(|error| {
                    format!("invalid typeset identifier {typeset_text:?}: {error}")
                })?;
                let typeset = catalog
            ...
            ConfigurationKind::Template => {
                let id = gitserious_core::TemplateId::new(identity).map_err(|error| {
                    format!("invalid template identifier {identity:?}: {error}")
                })?;
